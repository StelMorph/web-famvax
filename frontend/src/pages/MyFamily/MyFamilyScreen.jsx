...
const MyFamilyPopulated = ({ profiles, stats, onAddProfile, onSelectProfile }) => (
     <div className="content-wrapper">
        ...
        <div className="stat-card-container">
            <StatCard icon={faUsers} title="Family Members" count={profiles.length} colorClass="icon-bg-blue" />
            <StatCard icon={faCalendarAlt} title="Upcoming" count={stats.upcomingCount} colorClass="icon-bg-yellow" />
            <StatCard icon={faShieldVirus} title="Completed" count={stats.completedCount} colorClass="icon-bg-green" />
        </div>
        <div className="profile-grid-new">
            {profiles.map(profile => (
                <ProfileCard key={profile.profileId} profile={profile} onSelect={() => onSelectProfile(profile.profileId)} />
            ))}
        </div>
    </div>
);
    ...
    return (
        <div className="screen active my-family-page">
           {allProfiles && allProfiles.length > 0 ? (
               <MyFamilyPopulated profiles={allProfiles} stats={stats} onAddProfile={handleAddProfile} onSelectProfile={handleSelectProfile} />
           ) : (
