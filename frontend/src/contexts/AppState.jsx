...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'NAVIGATE':
      return {
        ...state,
        history: [state.activeScreen, ...state.history],
    ...
    case 'GO_BACK': {
      const [lastScreen, ...rest] = state.history;
      return {
        ...state,
...
export const AppProvider = ({ children }) => {
  ...
  const maybeBlockLeavingAIReview = React.useCallback(
    (proceed) => {
    ...
    [appState.activeScreen, appState.reviewDirty, dispatch, navGuardRef],
  );

  const navigateTo = React.useCallback(
    (screen, params = {}, options = {}) => {
      if (screen === 'auth-screen' && hasValidTokenSync()) {
        return;
      }
    ...
    [dispatch, maybeBlockLeavingAIReview, navGuardRef],
  );

  const goBack = React.useCallback(() => {
    const proceed = () => dispatch({ type: 'GO_BACK' });
    ...
    proceed();
  }, [maybeBlockLeavingAIReview, navGuardRef]);
  ...
  }, [appState.areDetailsLoading, dispatch]);
  ...
  const value = useMemo(
    ...
    [appState, navigateTo, goBack, fetchDetailedData],
  );
