...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    ...
    try {
        ...
        const canDelete = shareResult.Item.ownerId === userId || shareResult.Item.inviteeEmail === userEmail;
        if (!canDelete) {
            return { statusCode: 403, headers: CORS_HEADERS, body: JSON.stringify({ message: 'Forbidden: You cannot modify this share.' }) };
        }

        await docClient.send(new DeleteCommand({ TableName: process.env.SHARE_INVITES_TABLE_NAME!, Key: { shareId } }));
        return { statusCode: 204, headers: CORS_HEADERS, body: '' };
    } catch (error: any) {
