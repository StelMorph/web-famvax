...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    ...
    try {
        ...
        if (!profileId || !ownerId || !ownerEmail) {
            return { statusCode: 401, headers: CORS_HEADERS, body: JSON.stringify({ message: 'User is not authenticated' }) };
        }
        if (!inviteeEmail || !role || !['Viewer', 'Editor'].includes(role)) {
            return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ message: 'A valid inviteeEmail and role (Viewer/Editor) are required.' }) };
        }
        ...
        if (!profileResult.Item) {
            return { statusCode: 404, headers: CORS_HEADERS, body: JSON.stringify({ message: 'Profile not found.' }) };
        }
        // Now that we know .Item exists, we can safely check ownership.
        if (profileResult.Item.userId !== ownerId) {
            return { statusCode: 403, headers: CORS_HEADERS, body: JSON.stringify({ message: 'Forbidden: You do not own this profile.' }) };
        }

        // --- 3. Find the invited user in Cognito to get their permanent ID ---
        const listUsersCmd = new ListUsersCommand({ UserPoolId: process.env.USER_POOL_ID!, Filter: `email = "${inviteeEmail}"`, Limit: 1 });
        const cognitoResult = await cognitoClient.send(listUsersCmd);
        const invitee = cognitoResult.Users?.[0];

        if (!invitee?.Username) {
            return { statusCode: 404, body: JSON.stringify({ message: "Invited user does not exist. Please ask them to sign up first." }) };
        }
        ...
        if (existingShare) {
            ...
            if (existingShare.role === role) {
                return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify({ message: `An invitation with the '${role}' role has already been sent.` }) };
            }
            const updateCmd = new UpdateCommand({
                TableName: process.env.SHARE_INVITES_TABLE_NAME!,
                Key: { shareId: existingShare.shareId },
                UpdateExpression: 'set #role = :role, #status = :status, #updatedAt = :ts',
                ExpressionAttributeNames: { '#role': 'role', '#status': 'status', '#updatedAt': 'updatedAt' },
                ExpressionAttributeValues: { ':role': role, ':status': 'PENDING', ':ts': new Date().toISOString() },
            });
            await docClient.send(updateCmd);
            return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify({ message: `Invitation for ${inviteeEmail} updated to ${role}.` }) };
            ...
            const createCmd = new PutCommand({
                TableName: process.env.SHARE_INVITES_TABLE_NAME!,
                Item: {
                    shareId, profileId, ownerId, inviteeId, inviteeEmail, role,
                    status: 'PENDING',
            ...
            return { statusCode: 201, headers: CORS_HEADERS, body: JSON.stringify({ message: 'Invite sent successfully.' }) };
        }
    } catch (error: any) {
