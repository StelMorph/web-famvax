...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    ...
    try {
        ...
        if (!canEdit) {
            return { statusCode: 403, headers: CORS_HEADERS, body: JSON.stringify({ message: 'Forbidden: You do not have permission to add records to this profile.' }) };
        }
        ...
        return { statusCode: 201, headers: CORS_HEADERS, body: JSON.stringify({ vaccineId, ...data }) };
    } catch (error: any) {
