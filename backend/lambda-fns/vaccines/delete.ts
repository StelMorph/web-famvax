...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    ...
    try {
        ...
        if (!canEdit) {
            return { statusCode: 403, headers: CORS_HEADERS, body: JSON.stringify({ message: 'Forbidden: You do not have permission to delete this record.' }) };
        }
        
        await docClient.send(new DeleteCommand({ TableName: process.env.VACCINES_TABLE_NAME!, Key: { vaccineId } }));
        return { statusCode: 204, headers: CORS_HEADERS, body: '' };
    } catch (error: any) {
